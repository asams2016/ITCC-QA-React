...
describe('<ErrorAlert />', () => {
	it('allows us to set props', () => {
		const wrapper = mount(<ErrorAlert testProp='testPropVal' store={store} />);
	...
	it('renders a button that closes the alert', () => {
		...
		const wrapper = mount(<ErrorAlert store={store} closeAlert={closeClick} />);
	...
	it('renders a header', () => {
		const wrapper = mount(<ErrorAlert store={store} />);
